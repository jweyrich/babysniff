...
#define LOG_FATAL(...) \
	LOG_SELECT_FUNC(log_printf_level_narg_, LOG_NARG(LOG_ARG_0_4_5_RSEQ,__VA_ARGS__), \
...
#define LOG_TRACE \
	log_printf_level_narg_5(__FILE__, __LINE__, LOGLEVEL_TRACE, "%s called", __func__) //__PRETTY_FUNCTION__

#define LOG_PRINTF(...) \
	do { \
		LOG_SELECT_FUNC(log_printf_narg_, LOG_NARG(LOG_ARG_0_1_2_RSEQ,__VA_ARGS__), \
			__VA_ARGS__); \
	} while (0)

#define LOG_PRINTF_INDENT(indent, ...) \
	do { \
		LOG_SELECT_FUNC(log_printf_indent_narg_, LOG_NARG(LOG_ARG_0_3_4_RSEQ,__VA_ARGS__), \
			indent, " ", __VA_ARGS__); \
	} while (0)
